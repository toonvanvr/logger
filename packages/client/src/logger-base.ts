...
export class LoggerBase {
  ...
  // ─── Protected internals ────────────────────────────────────────
  protected base(severity: SeverityType): LogEntry {
    return baseFields(
  ...
  protected log(severity: SeverityType, message: string, meta?: Record<string, unknown>): void {
    this.enqueue(buildTextEntry(this.base(severity), message, meta ? stringifyTags(meta) : undefined));
  }

  protected logError(severity: SeverityType, err: Error, meta?: Record<string, unknown>): void {
    this.enqueue({
  ...
  protected enqueue(entry: LogEntry): void {
    if (this._nextSticky) { entry = { ...entry, sticky: true }; this._nextSticky = false; }
    if (this._nextId) { entry = { ...entry, id: this._nextId }; this._nextId = undefined; }
    if (this._nextAfterId) { entry = { ...entry, after_id: this._nextAfterId }; this._nextAfterId = undefined; }
    if (this._nextBeforeId) { entry = { ...entry, before_id: this._nextBeforeId }; this._nextBeforeId = undefined; }
