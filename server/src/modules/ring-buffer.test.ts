...
describe('RingBuffer', () => {
  ...
  it('evicts oldest when maxBytes exceeded', () => {
    // Each entry is roughly 200+ bytes (estimateBytes fast path: 200 + text.length*2)
    // With 5-char text: 200 + 10 = 210 bytes per entry
    const buf = new RingBuffer(1000, 500); // 500 bytes max ~2 entries

    buf.push(makeEntry({ id: 'e1', text: 'hello' }));
    buf.push(makeEntry({ id: 'e2', text: 'world' }));

    const sizeAfter2 = buf.size;
    // Push a third — should evict to fit
    buf.push(makeEntry({ id: 'e3', text: 'test!' }));
  ...
  it('upsert replaces existing entry by ID when replace=true', () => {
    ...
    buf.push(makeEntry({ id: 'e1', text: 'original' }));
    expect(buf.get('e1')!.text).toBe('original');

    buf.upsert(makeEntry({ id: 'e1', text: 'replaced', replace: true }));
    expect(buf.size).toBe(1);
    expect(buf.get('e1')!.text).toBe('replaced');
  });

  it('upsert appends when ID not found', () => {
    const buf = new RingBuffer(100, 1024 * 1024);

    buf.upsert(makeEntry({ id: 'e1', text: 'first' }));
    buf.upsert(makeEntry({ id: 'e2', text: 'second' }));
  ...
  it('upsert appends when replace=false even if ID exists', () => {
    ...
    buf.push(makeEntry({ id: 'e1', text: 'original' }));
    buf.upsert(makeEntry({ id: 'e2', text: 'new' }));
    expect(buf.size).toBe(2);
  });
